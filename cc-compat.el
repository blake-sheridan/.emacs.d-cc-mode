;;; cc-convert.el --- convertion from c-mode vars to cc-mode vars
...
;; LCD Archive Entry:
;; cc-convert|Barry A. Warsaw|bwarsaw@cen.com
;; |convert from c-mode.el indentation vars to cc-mode.el indentation vars
;; |$Date$|$Revision$|
...
(defun cc-block-intro-offset (langelem)
  ;; taken directly from calculate-c-indent lossage
  (save-excursion
    (goto-char (cdr langelem))
    ;; If no previous statement, indent it relative to line brace is
    ;; on.  For open brace in column zero, don't let statement start
    ;; there too.  If c-indent-level is zero, use c-brace-offset +
    ;; c-continued-statement-offset instead.  For open-braces not the
    ;; first thing in a line, add in c-brace-imaginary-offset.
    (+ (if (and (bolp) (zerop c-indent-level))
	   (+ c-brace-offset c-continued-statement-offset)
	 c-indent-level)
       ;; Move back over whitespace before the openbrace.  If
       ;; openbrace is not first nonwhite thing on the line, add the
       ;; c-brace-imaginary-offset.
       (progn (skip-chars-backward " \t")
	      (if (bolp) 0 c-brace-imaginary-offset))
       ;; If the openbrace is preceded by a parenthesized exp, move to
       ;; the beginning of that; possibly a different line
       (progn
